...
function AddDealers({ open, onClose, onDealerAdded }) {
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post('https://namami-infotech.com/SAFEGUARD/src/dealer/add_dealers.php', {
                dealerName,
                address,
                contactInfo,
                latLong,
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            <DialogTitle style={{ backgroundColor: "#8d0638ff", color: "white" }}>Add Dealer</DialogTitle>
            <DialogContent>
                <TextField
                    label="Dealer Name"
                    value={dealerName}
            ...
            <DialogActions>
                ...
                <Button onClick={handleSubmit} color="#8d0638ff">
                    Submit
                </Button>
            </DialogActions>
        </Dialog>
