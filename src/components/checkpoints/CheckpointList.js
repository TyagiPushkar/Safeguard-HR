...
function CheckpointList() {
    ...
    const fetchCheckpoints = async () => {
        try {
            const response = await axios.get('https://namami-infotech.com/SAFEGUARD/src/menu/get_checkpoints.php');
  ...
  const addCheckpoint = () => {
    ...
    return (
        <div>
            ...
            <Typography variant="h6" gutterBottom style={{ marginBottom: '20px', color: '#8d0638ff' }}>
                Checkpoints List
            </Typography>
 <Button variant="contained" color="#8d0638ff" onClick={addCheckpoint} sx={{ mb: 2 }} style={{ backgroundColor: "#8d0638ff" }}>
                Add Checkpoint
                </Button> 
                </div>
