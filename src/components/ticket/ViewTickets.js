...
function ViewTickets() {
    ...
    const fetchTickets = async () => {
        try {
            const [ticketsResponse, employeesResponse] = await Promise.all([
                axios.get('https://namami-infotech.com/SAFEGUARD/src/support/get_ticket.php', {
    ...
    const handleStatusChange = async (id, status) => {
        try {
            const response = await axios.post('https://namami-infotech.com/SAFEGUARD/src/support/update_status.php', {
                id,
                status,
            });
    ...
    return (
        <Box>
            <Button
                ...
                color="#8d0638ff"
                onClick={exportToCsv}
                style={{ marginBottom: '16px', backgroundColor: "#8d0638ff", float: "right" }}
            >
                Export CSV
            </Button>
                                        ...
                                        {user.role === 'HR' && (
                                            <TableCell>
                                                {ticket.Status === 'Pending' && (
                                                    <>
                                                        <IconButton onClick={() => handleStatusChange(ticket.id, 'Approved')} color="#8d0638ff">
                                                            <CheckIcon />
                                                        </IconButton>
