    ...
    useEffect(() => {
        if (user.role === 'HR') {
            const fetchEmployees = async () => {
                try {
                    const response = await axios.get(`https://namami-infotech.com/SAFEGUARD/src/employee/list_employee.php?Tenent_Id=${user.tenent_id}`);
...
useEffect(() => {
   ...
   const fetchVisits = async () => {
    ...
    try {
        ...
        const url = `https://namami-infotech.com/SAFEGUARD/src/visit/plan.php?role=HR&empId=${selectedEmpId}&date=${formattedDate}`;
...
const handleViewImage = async (visitId) => {
        try {
            const response = await axios.get(`https://namami-infotech.com/SAFEGUARD/src/visit/view_one_visit.php?visit_id=${visitId}`);
    ...
    return (
        <>
            ...
            <Button
                variant="contained"
                style={{ marginBottom: 20, backgroundColor: "#8d0638ff", color: "white" }}
                onClick={fetchVisits}
            >
                Go
            </Button>
            ...
            {/* <Button
                variant="contained"
                style={{ backgroundColor: "#8d0638ff", color: "white", marginTop: 10 }}
